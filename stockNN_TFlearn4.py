...
def main():
    ...
    for seqid in range(0, total_loop):
        ...
        hpDict['TrainRowNum'] = X_train.shape[0]  # get the training data record row numbers, to be written to result
        # try pca
        # from sklearn.decomposition import PCA
        # pca = PCA()
        # pca.fit(X_train)
        # log("pca.components_ is:\n")
        # print(pca.components_)
        # log("pca.explained_variance_ is:\n")
        # print(pca.explained_variance_)
        #
        # exit(2)  from the result, the first

        # X_test = pca.transform(X_test)

        # try feature selections, only select the best 132 features, which is 80% of total feature numbers
        # select all 174 features,which is 100% input data
        inputLayerNodeNum = 180
        log("apply featureSelection but do NOT use it to pick up %d best features" %inputLayerNodeNum)
